...
## 2026-06-11 — Feature 5: api-integration (mercurial-hr-chat)
- **Agente:** Claude Sonnet 4.6 (tech-lead) → developer → reviewer → qa → developer (fix mutante).
- **Recorrido:** feature `in_progress` retomada → TDD estricto Rojo-Verde-Refactor (15 escenarios @s1–@s15) → reviewer **APPROVED** (15/15 escenarios cubiertos, 76 tests pasan) → qa **FAIL** (mutante sobreviviente en `httpError.ts`: `&&` → `||`) → developer agrega test para matar mutante → qa **PASS** (100%).
- **Cambios:** `src/services/hrService.ts` (fetch real a `/api/query`), `src/services/ragService.ts` (fetch real a `/api/ingest`, guard lista vacía, `file.text()`), `src/services/httpError.ts` (utilidad `extractBackendError` extraída en refactor), `tests/ApiIntegration.test.ts` (16 tests: @s1–@s15 + test de mutante).
- **Trazabilidad @s → test:**
  - @s1 envía POST /api/query con { question }
  - @s2 retorna result de respuesta 200
  - @s3–@s4 lanza Error con mensaje del backend (4xx/5xx)
  - @s5 relanza como Error nativo (no AxiosError)
  - @s6 URL desde VITE_API_BASE_URL
  - @s7 envía POST /api/ingest con { documents }
  - @s8 lee archivos con file.text()
  - @s9–@s10 resuelve void en 200/204
  - @s11–@s12 lanza Error con mensaje del backend (4xx/5xx)
  - @s13 relanza como Error nativo (no AxiosError)
  - @s14 rechaza sin request si lista vacía
  - @s15 URL desde VITE_API_BASE_URL
- **Verificación:** 76/76 tests pasan. Mutación 100% (3/3 archivos).
- **Cierre:** feature 5 marcada `done`. Todas las features del proyecto completadas.
