...
| `promptsService.list` invoca `getDocAgentBaseUrl()` y arma la URL con el valor devuelto, sea cual sea el entorno | `promptsService construye la URL con getDocAgentBaseUrl(), sea cual sea el entorno` (renombrado por feature 16 — antes: `getDocAgentBaseUrl — usa http://localhost:8000 como fallback`) | `tests/PromptsService.test.ts` |
